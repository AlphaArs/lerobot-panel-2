    ...
    const connect = () => {
      ...
      socket.onmessage = (event) => {
        ...
        try {
          ...
          type CalibrationRangeRow = { name: string; min: number; pos: number; max: number };
          const incomingRanges = (payload.ranges || []) as CalibrationRangeRow[];
          setCalibrationRanges(
            incomingRanges.reduce<Record<string, CalibrationRangeRow>>((acc, row) => {
              acc[row.name] = row;
              return acc;
            }, {})
          );
