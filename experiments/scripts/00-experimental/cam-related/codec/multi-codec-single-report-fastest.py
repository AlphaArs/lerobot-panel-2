...
def frame_signature(frame):
    # Very cheap "is this a new frame" signature:
    # downscale -> gray -> sum.
    small = cv2.resize(frame, (32, 18), interpolation=cv2.INTER_AREA)
    gray = cv2.cvtColor(small, cv2.COLOR_BGR2GRAY)
    return int(gray.sum())

def measure_fps_fast(cap, seconds=0.35, warmup=5, yield_sleep=0.001):
    # Warmup
    last_sig = None
    ...
    while time.perf_counter() - t0 < seconds:
        ...
        if sig != last_sig:
            ...
            # If we’re seeing duplicates, yield so we don’t spin
            time.sleep(yield_sleep)
...
def test_mode(cam_idx, label, w, h, fps, fourcc):
    ...
    measured = measure_fps_fast(cap, seconds=0.35, warmup=5, yield_sleep=0.001)
